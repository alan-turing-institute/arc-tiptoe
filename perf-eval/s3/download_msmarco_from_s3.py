...
parser.add_argument(
    "-p", "--path", help="Path to write files", type=str, default="/home/ubuntu"
)
...
wget.download(
    ...
    out="%s/msmarco_checkpoints/" % path,
)
wget.download(
...
with gzip.open(
    "%s/msmarco_checkpoints/msmarco-docdev-qrels.tsv.gz" % path, "rb"
) as f_in:
    with open("%s/msmarco_checkpoints/msmarco-docdev-qrels.tsv" % path, "wb") as f_out:
        shutil.copyfileobj(f_in, f_out)
with gzip.open(
    "%s/msmarco_checkpoints/msmarco-docdev-queries.tsv.gz" % path, "rb"
) as f_in:
    with open(
        "%s/msmarco_checkpoints/msmarco-docdev-queries.tsv" % path, "wb"
    ) as f_out:
        shutil.copyfileobj(f_in, f_out)
        shutil.copyfileobj(f_in, f_out)
