...
def find_nearest_clusters(cluster_index, query, num_clusters):
    """Find the nearest clusters for the given query using FAISS index."""
...
def find_nearest_clusters_from_file(centroids, query_embed, num_clusters):
    """Find the nearest clusters for the given query using precomputed centroids."""
...
def main():
    """Main function to read input, embed text, and find nearest clusters."""
    if len(sys.argv) != 3:
        raise ValueError(f"Usage: {sys.argv[0]} preamble num_clusters")
    ...
    for line in sys.stdin:
        ...
        # start2 = time.time()
        v = model.encode(line)
        v = numpy.round(v * (1 << prec)).astype(
            "int"
        )  # WHY NOT JUST KEEP MORE PRECISION EARLIER ON?
        # end2 = time.time()
        ...
        out = numpy.clip(numpy.round(numpy.matmul(v, components) / 10), -16, 15).astype(
            "int"
        )
        sys.stdout.write(
            json.dumps({"Cluster_index": int(result), "Emb": out.tolist()})
        )
        sys.stdout.flush()
        # end4 = time.time()
        # print("PCA: ", end4-end3)
