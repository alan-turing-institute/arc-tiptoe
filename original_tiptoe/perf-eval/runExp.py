...
def provision():
    ...
    startEc2Instance(
        ...
        properties["ami_id"],
        key,
        properties["coordinator_instance_type"],
        [properties["security"]],
        properties["placement"],
    ...
    startEc2Instance(
        ...
        properties["ami_id"],
        key,
        properties["lat_client_instance_type"],
        [properties["security"]],
        properties["placement"],
        name=getLatClientName(),
        disk_size=properties["disk_size"],
    )
    ...
    startEc2Instance(
        ...
        properties["ami_id"],
        key,
        properties["tput_client_instance_type"],
        [properties["security"]],
        properties["placement"],
        name=getTputClientName(),
        disk_size=properties["disk_size"],
    )
    ...
    for i in range(properties["num_embed_servers"]):
        machines["embed_server_ip_address"].append(
            getIpByName(conn, getEmbedServerName(i))
        )
        machines["embed_server_private_ip_address"].append(
            getPrivateIpByName(conn, getEmbedServerName(i))
        )
    ...
    for i in range(properties["num_url_servers"]):
        ...
        machines["url_server_private_ip_address"].append(
            getPrivateIpByName(conn, getUrlServerName(i))
        )
    print(machines["url_server_ip_address"])

    machines["coordinator_ip_address"] = getIpByName(conn, getCoordinatorName())
    machines["coordinator_private_ip_address"] = getPrivateIpByName(
        conn, getCoordinatorName()
    )

    machines["lat_client_ip_address"] = getIpByName(conn, getLatClientName())
    machines["lat_client_private_ip_address"] = getPrivateIpByName(
        conn, getLatClientName()
    )

    machines["tput_client_ip_address"] = getIpByName(conn, getTputClientName())
    machines["tput_client_private_ip_address"] = getPrivateIpByName(
        conn, getTputClientName()
    )
...
def setup_machine(ip_addr):
    ...
    executeRemoteCommand(
        getHostName(ip_addr), "mkdir -p ~/.aws", key=properties["secret_key_path"]
    )
    sendFile(
        "~/.aws/credentials",
        getHostName(ip_addr),
    ...
    executeRemoteCommand(
        getHostName(ip_addr),
        "ssh-keyscan github.com >> ~/.ssh/known_hosts; git clone git@github.com:ahenzinger/tiptoe.git; source /home/ubuntu/.profile; cd tiptoe; go mod download",
        key=properties["secret_key_path"],
        flags="-A",
    )
...
def download_embedding_data(ip_addr, shard_idx):
    ...
    if args.image_search:
        executeRemoteCommand(
            getHostName(ip_addr),
            "cd tiptoe/perf-eval/s3; python3 img_download_from_s3.py embedding %d"
            % start,
            key=properties["secret_key_path"],
        )
        ...
        executeRemoteCommand(
            getHostName(ip_addr),
            "cd tiptoe/perf-eval/s3; python3 text_download_from_s3.py embedding %d"
            % start,
            key=properties["secret_key_path"],
        )
        for i in range(start + 1, end):
    ...
    executeRemoteCommand(
        getHostName(ip_addr), "rm ~/.aws/credentials", key=properties["secret_key_path"]
    )
...
def download_url_data(ip_addr, shard_idx):
    ...
    if args.image_search:
        executeRemoteCommand(
            getHostName(ip_addr),
            "cd tiptoe/perf-eval/s3; python3 img_download_from_s3.py url %d" % start,
            key=properties["secret_key_path"],
        )
        for i in range(start + 1, end):
            executeRemoteCommand(
                getHostName(ip_addr),
                "cd tiptoe/perf-eval/s3; python3 img_download_from_s3.py url %d" % i,
                key=properties["secret_key_path"],
            )
    else:
        executeRemoteCommand(
            getHostName(ip_addr),
            "cd tiptoe/perf-eval/s3; python3 text_download_from_s3.py url %d" % start,
            key=properties["secret_key_path"],
        )
        for i in range(start + 1, end):
            executeRemoteCommand(
                getHostName(ip_addr),
                "cd tiptoe/perf-eval/s3; python3 text_download_from_s3.py url %d" % i,
                key=properties["secret_key_path"],
            )
    executeRemoteCommand(
        getHostName(ip_addr), "rm ~/.aws/credentials", key=properties["secret_key_path"]
...
def download_coordinator_data(ip_addr):
    ...
    if args.image_search:
        executeRemoteCommand(
            getHostName(ip_addr),
            "cd tiptoe/perf-eval/s3; python3 img_download_from_s3.py coordinator 0",
            key=properties["secret_key_path"],
        )
    else:
        executeRemoteCommand(
            getHostName(ip_addr),
            "cd tiptoe/perf-eval/s3; python3 text_download_from_s3.py coordinator 0",
            key=properties["secret_key_path"],
        )
    executeRemoteCommand(
        getHostName(ip_addr), "rm ~/.aws/credentials", key=properties["secret_key_path"]
...
def download_client_data(ip_addr):
    ...
    if args.image_search:
        executeRemoteCommand(
            getHostName(ip_addr),
            "cd tiptoe/perf-eval/s3; python3 img_download_from_s3.py client 0",
            key=properties["secret_key_path"],
        )
    else:
        executeRemoteCommand(
            getHostName(ip_addr),
            "cd tiptoe/perf-eval/s3; python3 text_download_from_s3.py client 0",
            key=properties["secret_key_path"],
        )
    executeRemoteCommand(
        getHostName(ip_addr), "rm ~/.aws/credentials", key=properties["secret_key_path"]
...
def setupAll():
    ...
    with concurrent.futures.ThreadPoolExecutor(
        max_workers=properties["num_embed_servers"]
    ) as executor:
        future_to_res = [
            executor.submit(setup_machine, machines["embed_server_ip_address"][i])
            for i in range(properties["num_embed_servers"])
        ]
    ...
    with concurrent.futures.ThreadPoolExecutor(
        max_workers=properties["num_url_servers"]
    ) as executor:
        future_to_res = [
            executor.submit(setup_machine, machines["url_server_ip_address"][i])
            for i in range(properties["num_url_servers"])
        ]
        for future in concurrent.futures.as_completed(future_to_res):
            future.result()
    ...
    with concurrent.futures.ThreadPoolExecutor(
        ...
        future_to_res = [
            executor.submit(
                download_embedding_data, machines["embed_server_ip_address"][i], i
            )
            for i in range(properties["num_embed_servers"])
        ]
        for future in concurrent.futures.as_completed(future_to_res):
    ...
    with concurrent.futures.ThreadPoolExecutor(
        ...
        future_to_res = [
            executor.submit(download_url_data, machines["url_server_ip_address"][i], i)
            for i in range(properties["num_url_servers"])
        ]
        for future in concurrent.futures.as_completed(future_to_res):
            future.result()
...
def setupExp():
    ...
    for j in range(num_logical):
        for i in range(properties["num_embed_servers"]):
            at = num_logical * i + j
            executeRemoteCommand(
                getHostName(machines["embed_server_ip_address"][i]),
                (
                    "source /home/ubuntu/.profile; cd tiptoe/search; if pgrep server; then pkill -f server; fi; nohup go run . emb-server %d > /dev/null 2>&1 &"
                )
                % at,
                key=properties["secret_key_path"],
            )
        ...
        for i in range(properties["num_url_servers"]):
            at = num_logical * i + j
            executeRemoteCommand(
                getHostName(machines["url_server_ip_address"][i]),
                (
                    "source /home/ubuntu/.profile; cd tiptoe/search; if pgrep server; then pkill -f server; fi; nohup go run . url-server %d > /dev/null 2>&1 &"
                )
                % at,
                key=properties["secret_key_path"],
            )
    ...
    ip_string = (
        " ".join(np.repeat(machines["embed_server_private_ip_address"], num_logical))
        + " "
        + " ".join(np.repeat(machines["url_server_private_ip_address"], num_logical))
    )
    coordinator_out = executeRemoteCommandWithOutputReturn(
        getHostName(machines["coordinator_ip_address"]),
        "source /home/ubuntu/.profile; cd tiptoe/search; if pgrep coordinator; then pkill -f coordinator; fi; nohup go run . coordinator %d %d %s %s > /dev/null 2>&1 &"
        % (
            properties["num_embed_servers"] * num_logical,
            properties["num_url_servers"] * num_logical,
            ip_string,
            flag,
        ),
        key=properties["secret_key_path"],
...
def runLatencyExp():
    ...
    client_out = executeRemoteCommandWithOutputReturn(
        getHostName(machines["lat_client_ip_address"]),
        "source /home/ubuntu/.profile; cd tiptoe/search; ./wan.sh; go run . client-latency %s %s"
        % (machines["coordinator_private_ip_address"], flag),
        key=properties["secret_key_path"],
    )
    ...
    fn = (
        prefix
        + str(properties["num_embed_servers"])
        + "-"
        + str(properties["num_url_servers"])
        + "-"
        + str(properties["servers_per_machine"])
        + "-latency.log"
    )
    with open(fn, "w") as f:
        f.write(client_out)
...
def runTputExp():
    ...
    client_out = executeRemoteCommandWithOutputReturn(
        getHostName(machines["tput_client_ip_address"]),
        "source /home/ubuntu/.profile; cd tiptoe/search; go run . client-tput-embed %s %s"
        % (machines["coordinator_private_ip_address"], flag),
        key=properties["secret_key_path"],
    )
    ...
    client_out = executeRemoteCommandWithOutputReturn(
        getHostName(machines["tput_client_ip_address"]),
        "source /home/ubuntu/.profile; cd tiptoe/search; go run . client-tput-url %s"
        % (machines["coordinator_private_ip_address"]),
        key=properties["secret_key_path"],
    )
    ...
    client_out = executeRemoteCommandWithOutputReturn(
        getHostName(machines["tput_client_ip_address"]),
        "source /home/ubuntu/.profile; cd tiptoe/search; go run . client-tput-offline %s"
        % (machines["coordinator_private_ip_address"]),
        key=properties["secret_key_path"],
    )
