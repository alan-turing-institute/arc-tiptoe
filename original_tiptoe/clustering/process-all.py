...
def pca_contents(contents, data, pca_components):
    ...
    pca_output = []
    if IS_TEXT:
        pca_output = numpy.clip(
            numpy.round(numpy.matmul(data, pca_components) / 10), -16, 15
        )
    else:
        pca_output = numpy.clip(
            numpy.round(numpy.matmul(data, pca_components)), -16, 15
        )
    out = [
        (contents[i][0], ",".join(["%d" % ch for ch in pca_output[i]]), contents[i][2])
...
def cluster_by_url(contents, pca_components):
    ...
    if len(assignment_dict) == 1 and num_bundles > 1:
        ...
        for i in range(num_bundles):
            ...
            output_list.append(
                pca_contents(
                    contents[i * URLS_PER_BUNDLE : upper_bound],
                    data[i * URLS_PER_BUNDLE : upper_bound],
                    pca_components,
                )
            )
        return output_list
    ...
    for cluster in init_clusters:
        ...
        if get_size(assignment_dict[cluster]) > URL_MAX_SIZE:
            ...
            cluster_data = numpy.loadtxt(
                [elem[1] for elem in assignment_dict[cluster]], delimiter=","
            )
            if len(numpy.shape(cluster_data)) < 2:
                output_list.append(
                    pca_contents(
                        assignment_dict[cluster], [cluster_data], pca_components
                    )
                )
            else:
                output_list.append(
                    pca_contents(assignment_dict[cluster], cluster_data, pca_components)
                )
    return output_list
...
def split_cluster(contents, pca_components):
    ...
    if num_zeros == num_bundles - 1 and num_bundles > 0:
        for i in range(num_bundles):
            ...
            assignment_dict[i] = cluster_by_url(
                [contents[j] for j in range(i * AVG_BUNDLE_SIZE, upper_bound)],
                pca_components,
            )
    ...
    for i in range(len(centroids)):
        if i in assignment_dict:
            ...
            if len(assignment_dict[i - num_zeros]) > 0:
                centroids[i - num_zeros] = numpy.loadtxt(
                    [elem[1] for elem in assignment_dict[i]], delimiter=","
                ).mean(axis=0)
        else:
            num_zeros += 1
    ...
    for cluster in init_clusters:
        ...
        if len(assignment_dict[cluster]) > MAX_SIZE:
            (sub_centroids, sub_clustered_dict) = split_cluster(
                assignment_dict[cluster], pca_components
            )
            ...
            # Run URL clustering
            clustered_dict[cluster] = cluster_by_url(
                assignment_dict[cluster], pca_components
            )
...
def main():
    ...
    cluster_files = [
        "%s/clusters/cluster_%d.txt" % (CLUSTER_DIR, i) for i in range(NUM_CLUSTERS)
    ]
